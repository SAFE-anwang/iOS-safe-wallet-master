...
- (BRAssetHeaderView *)headerView {
    ...
    if (_headerView == nil) {
        _headerView = [[BRAssetHeaderView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 260)];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [BRWalletManager sharedInstance].wallet.balanceArray.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    static NSString *identifier = @"AssetCell";
    BRAssetCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[BRAssetCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
    }
    if(indexPath.row < [BRWalletManager sharedInstance].wallet.balanceArray.count) {
        BRBalanceModel *balanceModel = [BRWalletManager sharedInstance].wallet.balanceArray[indexPath.row];
        cell.balance = balanceModel;
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BRBalanceModel *balanceModel = [BRWalletManager sharedInstance].wallet.balanceArray[indexPath.row];
    historyVc.balanceModel = balanceModel;
...
#warning Language International
        if(self.headerView.addressLable.text.length == 0) {
            hud.label.text = NSLocalizedString(@"The address is empty and the copy failed.", nil);
        } else {
            hud.label.text = NSLocalizedString(@"Successful copy", nil);
            UIPasteboard *pboard = [UIPasteboard generalPasteboard];
            pboard.string = self.headerView.addressLable.text;
        }
...
- (void)createNewAddress {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        BRPaymentRequest *req = [BRPaymentRequest requestWithString:manager.wallet.newReceiveAddress];
        UIImage *image = nil;
